...
template <typename T> __device__ __host__ inline T hip_complex_exp(T z);

template<> __device__ __host__ inline hipFloatComplex hip_complex_exp(hipFloatComplex z)
{
    hipFloatComplex r;
    float t = expf(z.x);
...
template<> __device__ __host__ inline hipDoubleComplex hip_complex_exp(hipDoubleComplex z)
{
    hipDoubleComplex r;
    double t = exp(z.x);
    sincos(z.y, &r.y, &r.x);
...
void r2c_1d_fftw_ref(size_t const N, size_t batch, float const* inputs, cplx* outputs)
{
    ...
    plan = fftwf_plan_many_dft_r2c(rank, n, howmany,
        in, inembed, istride, idist, out, onembed, ostride, odist, FFTW_ESTIMATE);
    fftwf_execute(plan);
...
void r2c_1d_cpu(size_t const N, size_t batch, float const* inputs, cplx* outputs)
{
    ...
    for (auto i=0; i < batch; i++)
    {
        ...
        for (auto r = 1; r < N/2; r++)
        {
            ...
            outputs[output_idx_base + r] = zr       * (cplx(1, 0) - I * omega_n) * cplx(0.5f, 0.0f) +
                         conj(zt) * (cplx(1, 0) + I * omega_n) * cplx(0.5f, 0.0f) ;
        }
        outputs[output_idx_base + N/2] = cplx((float)(work_in_out[0][0] - work_in_out[0][1]), 0.0f);
    }
                                                  ...
                                                  size_t  output_distance )
{
    ...
    else if(idx_p <= input_size/4)
    {
        ...
        COMPLEX tmp = hip_complex_exp(COMPLEX(0.0f, (REAL)(-2.0f * M_PI * idx_p / input_size))); // omega_n for p
        tmp = (p + conj_q) * COMPLEX(0.5f, 0.0f) - (p - conj_q) * tmp * COMPLEX(0.0f, 0.5f);
        output[idx_p]= T(tmp.x, tmp.y);

        tmp = hip_complex_exp(COMPLEX(0.0f, (REAL)(-2.0f * M_PI * idx_q / input_size)));  // omega_n for q
...
// GPU host code
template <typename T>
void r2c_1d_post_process(size_t const N,  size_t batch,
                         T *d_input, T *d_output, T *d_twiddles,
                         size_t high_dimension, size_t input_stride, size_t output_stride,
...
{
    fftwf_complex *work_in_out  = (fftwf_complex*)fftwf_malloc(sizeof(fftwf_complex) * N/2 * batch);
    ...
    */
    ...
    for (auto i=0; i < batch; i++)
    {
        work_plan = fftwf_plan_dft_1d(N/2, &work_in_out[i*N/2], &work_in_out[i*N/2], FFTW_FORWARD, FFTW_ESTIMATE);
        fftwf_execute(work_plan);
    }
